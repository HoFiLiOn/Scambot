...
def start_cmd(message):
    ...
    bot.send_message(
        ...
        f"🔍 Команды для всех:\n"
        f"/search @username - проверить по юзернейму\n"
        f"/search 123456789 - проверить по ID\n"
        f"/help - помощь\n\n"
        f"👑 Админ-команды (только для админа):\n"
        f"/addscam @username - добавить в скам\n"
        f"/removescam @username - удалить из скам\n"
        f"/scamlist - список всех в скаме"
    )
    ...
    try:
        ...
        if query.isdigit():
            ...
            if found_user:
                ...
                bot.reply_to(message, f"❌ Пользователь с ID {user_id} еще не заходил в бота")
            return
...
if __name__ == "__main__":
    ...
    print("   /search")
    print("   /addscam (админ)")
    print("   /removescam (админ)")
    print("   /scamlist (админ)")
